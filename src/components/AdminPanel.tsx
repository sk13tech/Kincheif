...
const adminApp = initializeApp({
  apiKey: "AIzaSyDhdmRwQeYRu8r3zoUUTdXF8vLjiEleWSw", authDomain: "ecom-6586a.firebaseapp.com",
  projectId: "ecom-6586a", storageBucket: "ecom-6586a.firebasestorage.app",
  messagingSenderId: "956450721241", appId: "1:956450721241:web:26574ec21ef4ea12da9dc8",
}, 'admin-auth');
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-emerald-50/30 px-4">
      <form onSubmit={submit} className={`${card} w-full max-w-sm p-8 space-y-5`}>
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">PureHome Foods Management</p>
        </div>
        <div><label className={label}>Email</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} required className={input} placeholder="admin@purehome.com" /></div>
...
function Dashboard({ goTo }: { goTo: (p: string) => void }) {
  ...
  useEffect(() => {
    // Use onSnapshot to avoid flickering on re-mount
    const unsub = onSnapshot(collection(db, 'orders'), (snap) => {
      const st: any = { orders: snap.size, revenue: 0, products: 0, customers: 0, pending: 0, confirmed: 0, processing: 0, shipped: 0, delivered: 0, replacement: 0 };
      const all: any[] = []; const txnMap: Record<string, number> = {};
      snap.forEach(d => { const x = d.data(); if (x.status !== 'cancelled') { st.revenue += x.totalAmount || 0; st[x.status] = (st[x.status] || 0) + 1; } if (x.replacementRequested && x.replacementStatus !== 'reshipped') st.replacement++; all.push({ id: d.id, ...x });
        if (x.transactionId && x.transactionId !== 'GIFTCARD' && x.status !== 'cancelled') { txnMap[x.transactionId] = (txnMap[x.transactionId] || 0) + 1; }
      });
      all.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      setRecent(all.slice(0, 5));
      setDupTxns(Object.entries(txnMap).filter(([_, c]) => c > 1).map(([t]) => t));
      // Get product and contact counts separately
      getDocs(collection(db, 'products')).then(p => { st.products = p.size; getDocs(collection(db, 'contacts')).then(c => { st.customers = c.size; setS({ ...st }); setBusy(false); }); });
    });
    return () => unsub();
  }, []);
...
function Products() {
  ...
  const blank = { name: '', description: '', longDescription: '', price: '', originalPrice: '', image: '', images: '', category: '', tags: '', weight: '', ingredients: '', shelfLife: '', inStock: true, isNew: false, isBestseller: false, rating: '4.5', reviews: '0' };
  const startNew = () => { setForm({ ...blank }); setEdit('new'); };
  const startEdit = (p: any) => { setForm({ ...p, images: (p.images || []).join(', '), tags: (p.tags || []).join(', '), ingredients: (p.ingredients || []).join(', '), price: String(p.price || ''), originalPrice: String(p.originalPrice || ''), rating: String(p.rating || '4.5'), reviews: String(p.reviews || '0') }); setEdit(p); };

  const save = async () => {
    setBusy(true);
    const d: any = { ...form, price: Number(form.price) || 0, originalPrice: Number(form.originalPrice) || null, rating: Number(form.rating) || 4.5, reviews: Number(form.reviews) || 0,
    ...
    <div className="space-y-4">
      ...
      <div className={`${card} p-6 max-w-2xl`}>
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {F('Price ₹', 'price', { type: 'number' })}{F('MRP ₹', 'originalPrice', { type: 'number' })}{F('Weight', 'weight')}{F('Shelf Life', 'shelfLife')}
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div className={`${card} p-6 space-y-4`}><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1.5"><Phone className="h-3.5 w-3.5" /> Contact Information</p>
        {contactFields.map(([k, l]) => (<div key={k}><label className={label}>{l}</label><input value={cfg[k] || ''} onChange={e => setCfg({ ...cfg, [k]: e.target.value })} className={input} placeholder={k === 'contactPhone' ? '+91 98765 43210' : k === 'contactEmail' ? 'hello@purehome.com' : ''} /></div>))}
      </div>
...
function Layout() {
  ...
  const logoUrl = siteCfg.logoUrl || '/images/logo.png';
      ...
      <header className="fixed inset-x-0 top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-slate-200/80">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => signOut(adminAuth)} className="flex items-center gap-1.5 text-xs text-red-500 font-semibold hover:text-red-700 bg-red-50 px-3 py-1.5 rounded-xl"><LogOut className="h-3.5 w-3.5" /> Sign Out</button>
          </div>
        </div>
        {/* Main tabs */}
