...
function Products() {
  ...
  const blank = { name: '', description: '', longDescription: '', price: '', originalPrice: '', image: '', images: '', category: '', tags: '', weight: '', ingredients: '', shelfLife: '', maxOrderLimit: '', freeDeliveryPincodes: '', inStock: true, isNew: false, isBestseller: false };
  const startNew = () => { setForm({ ...blank }); setEdit('new'); };
  const startEdit = (p: any) => { setForm({ ...p, images: (p.images || []).join(', '), tags: (p.tags || []).join(', '), ingredients: (p.ingredients || []).join(', '), freeDeliveryPincodes: (p.freeDeliveryPincodes || []).join(', '), price: String(p.price || ''), originalPrice: String(p.originalPrice || ''), maxOrderLimit: String(p.maxOrderLimit || '') }); setEdit(p); };

  const save = async () => {
    setBusy(true);
    const d: any = { ...form, price: Number(form.price) || 0, originalPrice: Number(form.originalPrice) || null, maxOrderLimit: Number(form.maxOrderLimit) || null,
      freeDeliveryPincodes: form.freeDeliveryPincodes ? form.freeDeliveryPincodes.split(',').map((s: string) => s.trim()).filter(Boolean) : [],
    ...
    <div className="space-y-4">
      ...
      <div className={`${card} p-6 max-w-2xl`}>
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          {F('Price ₹', 'price', { type: 'number' })}{F('MRP ₹', 'originalPrice', { type: 'number' })}{F('Weight', 'weight')}{F('Shelf Life', 'shelfLife')}{F('Max Order Limit', 'maxOrderLimit', { type: 'number' })}{F('Free Delivery Pincodes (comma)', 'freeDeliveryPincodes', { full: true })}
...
function SiteConfig() {
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      {/* Show/Hide Toggles */}
      <div className={`${card} p-6 space-y-3`}><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Section Visibility</p>
        {[{ k: 'showHeroTitle', l: 'Hero Title' }, { k: 'showHeroBadge', l: 'Hero Badge' }, { k: 'showHeroSubtitle', l: 'Hero Subtitle' }, { k: 'showTestimonials', l: 'Testimonials' }].map(t => (
          <div key={t.k} className="flex items-center justify-between py-1.5">
            <span className="text-sm text-slate-700">{t.l}</span>
            <button onClick={() => setCfg({ ...cfg, [t.k]: cfg[t.k] === 'false' ? 'true' : cfg[t.k] === 'true' ? 'false' : 'false' })}
              className={`h-7 w-12 rounded-full p-0.5 transition-colors ${cfg[t.k] !== 'false' ? 'bg-emerald-500' : 'bg-slate-200'}`}>
              <div className={`h-6 w-6 rounded-full bg-white shadow-sm transition-transform ${cfg[t.k] !== 'false' ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
        ))}
      </div>
