...
export default function HeroSection({ onShopNow }: HeroProps) {
  ...
  const title = cfg.heroTitle || '';
  const subtitle = cfg.heroSubtitle || '';
  const badgeText = cfg.heroBadge || '';
  const heroImage = cfg.heroImage || '';
  const showTitle = cfg.showHeroTitle !== false;
  const showBadge = cfg.showHeroBadge !== false;
  const showSubtitle = cfg.showHeroSubtitle !== false;
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            {showBadge && badgeText && (
              <div className="inline-flex items-center gap-1.5 rounded-full border border-accent-green/30 bg-accent-green/5 px-3 py-1 mb-6">
                ...
                <span className="text-[10px] font-mono uppercase tracking-[.12em] text-accent-green">{badgeText}</span>
              </div>
            )}
            ...
            {showSubtitle && subtitle && <p className="mt-5 text-[15px] text-ink-500 max-w-md leading-relaxed">{subtitle}</p>}
