...
const upiApps = [
  { id: 'gpay', name: 'Google Pay', color: '#4285F4', prefix: 'tez://upi/pay' },
  { id: 'phonepe', name: 'PhonePe', color: '#5F259F', prefix: 'phonepe://pay' },
  { id: 'paytm', name: 'Paytm', color: '#00BAF2', prefix: 'paytmmp://pay' },
  { id: 'bhim', name: 'BHIM UPI', color: '#00897B', prefix: 'upi://pay' },
];
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            {payable === 0 ? (
              ...
              <>
                ...
                <div className="rounded-lg border border-sand-200 bg-white p-4">
                  <p className="text-[11px] font-mono uppercase tracking-[.12em] text-ink-400 mb-3">Open UPI App to Pay</p>
                  <div className="grid grid-cols-2 gap-2.5">
                    {upiApps.map(app => (
                      <a key={app.id} href={`${app.prefix}?pa=${encodeURIComponent(siteCfg.upiId || 'purehome@upi')}&pn=${encodeURIComponent(siteCfg.upiName || siteCfg.siteName || 'PureHome Foods')}&am=${payable}&cu=INR&tn=${encodeURIComponent('Order-' + (siteCfg.siteName || 'PureHome'))}`}
                        className="flex items-center gap-3 rounded-xl border border-sand-200 p-3.5 hover:border-sand-400 active:scale-[0.97]"
                        style={{ borderLeftColor: app.color, borderLeftWidth: 3 }}>
                        <div className="h-8 w-8 rounded-lg flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0" style={{ backgroundColor: app.color }}>{app.name.charAt(0)}</div>
                        <div><p className="text-[12px] font-semibold text-ink-800">{app.name}</p><p className="text-[9px] text-ink-400">Tap to open</p></div>
                      </a>
                    ))}
                  </div>
                </div>
