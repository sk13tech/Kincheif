            ...
            {payable === 0 ? (
              ...
              <>
                ...
                <div className="rounded-lg border border-sand-200 bg-white p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {upiApps.map(app => (
                      <a key={app.id} href={`${app.prefix}?pa=${encodeURIComponent(siteCfg.upiId || '')}&pn=${encodeURIComponent(siteCfg.upiName || siteCfg.siteName || '')}&am=${payable}&cu=INR&tn=${encodeURIComponent('Order-' + (siteCfg.siteName || ''))}`}
